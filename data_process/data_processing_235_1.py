...
data = pd.DataFrame(data_scale_2,columns=data.columns)
X_stand = data.values
print(X_stand.shape)
i = 1
for train_index, test_index in kf.split(X_stand):
    ...
    pd.DataFrame(X_train,columns=data.columns).to_csv('../data/data_splitClassifier_235/X_train{}.csv'.format(i),index=False)
    pd.DataFrame(X_test,columns=data.columns).to_csv('../data/data_splitClassifier_235/X_test{}.csv'.format(i),index=False)
    pd.DataFrame(X_val,columns=data.columns).to_csv('../data/data_splitClassifier_235/X_val{}.csv'.format(i),index=False)
    pd.DataFrame(y_train,columns=['label']).to_csv('../data/data_splitClassifier_235/y_train{}.csv'.format(i),index=False)
    pd.DataFrame(y_test,columns=['label']).to_csv('../data/data_splitClassifier_235/y_test{}.csv'.format(i),index=False)
    pd.DataFrame(y_val,columns=['label']).to_csv('../data/data_splitClassifier_235/y_val{}.csv'.format(i),index=False)
    print('第{}折交叉验证数据保存成功'.format(i))
    i=i+1