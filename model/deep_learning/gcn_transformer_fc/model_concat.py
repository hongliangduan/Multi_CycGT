...
from models import Transformer ,Model_TGCN
...
def main():
    for num in range(1, 11):
        ...
        # model_trans = nn.TransformerEncoderLayer(d_model=512,nhead=8)
        torch.cuda.empty_cache()
        ...
        for epoch in range(1, 2):
            ...
            # os.makedirs(f'./model/gcn_transformer_fc/{num}/gcn/', exist_ok=True)
            # os.makedirs(f'./model/gcn_transformer_fc/{num}/transformer/', exist_ok=True)
            # os.makedirs(f'./model/gcn_transformer_fc/{num}/tgcn/', exist_ok=True)
            ...
            # torch.save(model_trans, './model/gcn_transformer_fc/{}/tgcn/第{}个tgcn.pt'.format(num, epoch))



            # gcn_net = torch.load('./model/gcn_transformer_fc/{}/gcn/第{}个gcn.pt'.format(num, epoch))
            # model_trans = torch.load('./model/gcn_transformer_fc/{}/transformer/第{}个transformer.pt'.format(num, epoch))
            # model_trans= torch.load('./model/gcn_transformer_fc/{}/tgcn/第{}个tgcn.pt'.format(num, epoch))
            ...
            # os.makedirs(f'./pred_data/gcn_transformer_fc/{num}/test/', exist_ok=True)
            # pd.DataFrame(tt).to_csv('./pred_data/gcn_transformer_fc/{}/test/第{}次测试集预测值.csv'.format(num, epoch),
            #                         index=False)

            # y_true_val = pd.read_csv(PATH_x_test, usecols=['label']).values
            # y_true_val = y_true_val[:624]
            # y_true_val = y_true_val[:688]
            # t1, t2 = pd.DataFrame(val_list, columns=['predict']), pd.DataFrame(y_true_val, columns=['true'])
            # tt = pd.concat([t1, t2], axis=1)
            # os.makedirs(f'./pred_data/gcn_transformer_fc/{num}/val/', exist_ok=True)
            # pd.DataFrame(tt).to_csv('./pred_data/gcn_transformer_fc/{}/val/第{}次验证集预测值.csv'.format(num, epoch),
            #                         index=False)

            data = model_trans.named_parameters()

            result = [ (name, param) for name, param in data ]
            # target = ['encoders.1.feed_forward.fc2.weight', 'encoders.1.feed_forward.layer_norm.weight']
            target = ['encoders.1.feed_forward.layer_norm.weight']

            for name,param in model_tgcn.named_parameters():
                if name in target:
                # if 'weight' in name:
                    weights = param.data
                    print(weights)
            ...
            before_array = model_trans.before_list[0]

            # 循环遍历列表中的每个NumPy数组
            for arr in model_trans.before_list[1:]:

                # 合并当前数组到先前的数组上
                before_array = np.concatenate((before_array, arr))
            os.makedirs(f'./embeding_before/{num}/',exist_ok=True)
            pd.DataFrame(before_array).to_csv(f'./embeding_before/{num}/before_{epoch}.csv')
            model_trans.before_list.clear()


            after_array = model_trans.after_list[0]

            # 循环遍历列表中的每个NumPy数组
            for arr in model_trans.after_list[1:]:

                # 合并当前数组到先前的数组上
                after_array = np.concatenate((after_array, arr))
            os.makedirs(f'./embeding_after/{num}/',exist_ok=True)
            pd.DataFrame(after_array).to_csv(f'./embeding_after/{num}/after_{epoch}.csv')
            model_trans.after_list.clear()
