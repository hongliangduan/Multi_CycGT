...
def main():
    for num in range(10, 11):
        ...
        for epoch in range(1, 201):
            ...
            torch.save(gcn_net, './model/gcn/{}/gcn/第{}个gcn.pt'.format(num, epoch))
            torch.save(model_tgcn, './model/gcn/{}/g_fc/第{}个g_fc.pt'.format(num, epoch))
            ...
            pd.DataFrame(tt).to_csv('./pred_data/gcn/{}/test/第{}次测试集预测值.csv'.format(num, epoch),
                                    index=False)
            ...
            pd.DataFrame(tt).to_csv('./pred_data/gcn/{}/val/第{}次验证集预测值.csv'.format(num, epoch),
                                    index=False)
