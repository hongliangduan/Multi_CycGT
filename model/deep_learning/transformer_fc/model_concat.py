...
def main_():
    for num in range(1, 11):
        ...
        def collate(sample):
            _, list_num, labels, index = map(list, zip(*sample))
            return _, list_num, torch.tensor(labels), index

        train_X = pd.read_csv(PATH_x_train)
        y_train = get_data(train_X)
        train_data = list(zip(df_seq_train, list_num_train, y_train, [i for i in range(len(train_X))]))
        ...
        test_data = list(zip(df_seq_test, list_num_test, y_test, [i for i in range(len(test_X))]))
        ...
        val_data = list(zip(df_seq_val, list_num_val, y_val, [i for i in range(len(val_X))]))
        ...
        for epoch in range(1, 501):
            ...
            for i, (X, list_num,labels, index) in enumerate(train_loader_):
            ...
            def train_test_val(dataloader):
                ...
                model_trans.eval()
                model_tgcn.eval()
                for i, (X, list_num, labels, index) in enumerate(dataloader):
