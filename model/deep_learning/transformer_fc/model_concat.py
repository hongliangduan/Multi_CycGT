    ...
    for num in range(1, 2):
        ...
        for epoch in range(1, 201):
            ...
            for i, (X, list_num, graph, labels) in enumerate(train_loader_):
                ...
                y = model_trans(X)
                y = model_tgcn(y, list_num).to("cpu")
            ...
            # torch.save(model_trans, './model/transformer_fc/{}/transformer/第{}个transformer.pt'.format(num, epoch))
            # torch.save(model_tgcn, './model/transformer_fc/{}/t_fc/第{}个t_fc.pt'.format(num, epoch))
            ...
            os.makedirs(f'./model/transformer_fc/{num}/transformer/', exist_ok=True)
            os.makedirs(f'./model/transformer_fc/{num}/t_fc/', exist_ok=True)
            ...
            os.makedirs(f'./pred_data/transformer_fc/{num}/test/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/transformer_fc/{}/test/第{}次测试集预测值.csv'.format(num, epoch),
                                    index=False)
            ...
            tt = pd.concat([t1, t2], axis=1)
            os.makedirs(f'./pred_data/transformer_fc/{num}/val/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/transformer_fc/{}/val/第{}次验证集预测值.csv'.format(num, epoch),
                                    index=False)
