...
def main():
    for num in range(1, 11):
        ...
        for epoch in range(1, 201):
            ...
            os.makedirs(f'./model/transformer/{num}/transformer/', exist_ok=True)

            torch.save(model_trans, './model/transformer/{}/transformer/第{}个transformer.pt'.format(num, epoch))

            # model_trans = torch.load('./model/transformer/{}/transformer/第{}个transformer.pt'.format(num, epoch))
            ...
            os.makedirs(f'./pred_data/transformer/{num}/test/',exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/transformer/{}/test/第{}次测试集预测值.csv'.format(num, epoch),
                                    index=False)
            ...
            tt = pd.concat([t1, t2], axis=1)
            os.makedirs(f'./pred_data/transformer/{num}/val/')
            pd.DataFrame(tt).to_csv('./pred_data/transformer/{}/val/第{}次验证集预测值.csv'.format(num, epoch),
                                    index=False)
