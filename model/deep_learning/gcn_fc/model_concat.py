    ...
    for num in range(1, 11):
        ...
        for epoch in range(1, 201):
            ...
            for i, (X, list_num, graph, labels) in enumerate(train_loader_):
                ...
                y = model_tgcn(train_pred, list_num).to("cpu")
            ...
            os.makedirs(f'./model/gcn_fc/{num}/gcn/', exist_ok=True)
            os.makedirs(f'./model/gcn_fc/{num}/g_fc/', exist_ok=True)

            torch.save(gcn_net, './model/gcn_fc/{}/gcn/第{}个gcn.pt'.format(num, epoch))
            torch.save(model_tgcn, './model/gcn_fc/{}/g_fc/第{}个g_fc.pt'.format(num, epoch))
            ...
            def train_test_val(dataloader):
                ...
                for i, (X, list_num, graph, labels) in enumerate(dataloader):
                    ...
                    y = model_tgcn(pred, list_num).to("cpu")
            ...
            os.makedirs(f'./pred_data/gcn_fc/{num}/test/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/gcn_fc/{}/test/第{}次测试集预测值.csv'.format(num, epoch),
                                    index=False)
            ...
            tt = pd.concat([t1, t2], axis=1)
            os.makedirs(f'./pred_data/gcn_fc/{num}/val/', exist_ok=True)
            pd.DataFrame(tt).to_csv('./pred_data/gcn_fc/{}/val/第{}次验证集预测值.csv'.format(num, epoch),
                                    index=False)
