...
for num in range(1, 2):
    ...
    test_loader_val = DataLoader(test_data, batch_size=16, shuffle=False, collate_fn=collate, drop_last=True)
    ...
    for epoch in range(1):
        ...
        os.makedirs('./model/gcn_transfrom_fc/gcn/', exist_ok=True)
        os.makedirs('./model/gcn_transfrom_fc/transformer/', exist_ok=True)

        torch.save(gcn_net, './model/gcn_transfrom_fc/gcn/第{}个ggcn.pt'.format(epoch))
        torch.save(model_trans, './model/gcn_transfrom_fc/transformer/第{}个transsformer.pt'.format(epoch))
        ...
        os.makedirs(f'./pred_data/gcn_transfrom_fc_tv/', exist_ok=True)

        pd.DataFrame(tt).to_csv('./pred_data/gcn_transfrom_fc_tv/第{}次测试集预测值.csv'.format(epoch),index=False)


        val_epoch_loss, val_epoch_acc = 0, 0
        val_list = []
        for i, (X, list_num, graph, labels) in enumerate(test_loader_val):
            # print(i)
        ...
        print(f"epoch: {epoch + 1}, train_LOSS: {train_epoch_loss:.3f}, train_ACC: {train_epoch_acc:.3f}")
        print(f"epoch: {epoch + 1}, test_LOSS : {test_epoch_loss:.3f}, test_ACC : {test_epoch_acc:.3f}")
        print(f"epoch: {epoch + 1}, val_LOSS : {val_epoch_loss:.3f}, val_ACC : {val_epoch_acc:.3f}")

        print(f"预测值长度：{len(val_list)}")
        y_true_val = pd.read_csv(PATH_x_val, usecols=['label']).values
        print(f"val长度：{len(y_true_val)}")
        y_true_val = y_true_val[:625]
        # print(f"真实值长度：{y_true_val.shape}")
        t1, t2 = pd.DataFrame(val_list[:625], columns=['predict']), pd.DataFrame(y_true_val, columns=['true'])
        ...
        pd.DataFrame(tt).to_csv('./pred_data/gcn_transfrom_fc_tv/第{}次测试集预测值_val.csv'.format(epoch), index=False)
